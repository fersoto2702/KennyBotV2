...
const iconPath = path.join(__dirname, '../../assets/icons/unwarn.jpeg')

module.exports = {

    name: 'unwarn',
    aliases: ['removewarn', 'delwarn'],
    description: 'Elimina un warn a un usuario',
    category: 'grupos',
    adminOnly: true,
    groupOnly: true,
    ...
    async execute({ sock, from, msg }) {
        ...
        try {
            ...
            if (!target)
                return await sock.sendMessage(from, {
                    image: fs.readFileSync(iconPath),
                    caption: ui.warn('USUARIO REQUERIDO', 'Uso: /unwarn @usuario')
                })
            ...
            if (!warns[target])
                return await sock.sendMessage(from, {
                    image: fs.readFileSync(iconPath),
                    caption: ui.warn('SIN WARNS', 'Ese usuario no tiene advertencias.')
                })

            if (typeof warns[target].warns !== 'number') warns[target].warns = 0

            warns[target].warns -= 1
            if (warns[target].warns <= 0) delete warns[target]

            fs.writeFileSync(warnsPath, JSON.stringify(warns, null, 2))

            const total = warns[target]?.warns || 0
            const bar = '●'.repeat(total) + '○'.repeat(3 - total)

            logger.event(`Unwarn: ${target.split('@')[0]} → ${total}/3`)

            await sock.sendMessage(from, {
                image: fs.readFileSync(iconPath),
                caption: ui.success('WARN ELIMINADO', [
            ...
            logger.error(`Error unwarn: ${err.message}`)
        }
